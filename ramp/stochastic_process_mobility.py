...
def Stochastic_Process_Mobility(inputfile, country, year, full_year,start_month,end_month, num_pro):
    
    (peak_enlarg, mu_peak, s_peak, Year_behaviour, User_list, 
     Profile, Usage, Profile_user, Usage_user, num_profiles_user, 
     num_profiles_sim, dummy_days) = Initialise_inputs(inputfile, country, year, full_year,start_month,end_month,num_pro)
        ...
        for Us in User_list: #iterates for each User instance (i.e. for each user class)
            ...
            for i in range(Us.num_users): #iterates for every single user within a User class. Each single user has its own separate randomisation
                ...
                for App in Us.App_list: #iterates for all the App types in the given User class
                    ...
                    rand_dist = np.round(random.uniform(App.dist_tot,int(App.dist_tot*random_var_d))) 
                    
                    App.vel = App.func_dist/App.func_cycle * 60 
                    
                    rand_vel = np.maximum(20, np.round(random.uniform(App.vel,int(App.vel*random_var_v)))) #average velocity of the trip, minimum value is 20 km/h to get reasonable values from the power curve
                    
                    rand_time = int(np.round(rand_dist/rand_vel * 60))  #Function to calculate the total time based on total distance and average velocity 
                                ...
                                if tot_time > rand_time: #control to check when the total functioning time is reached. It will be typically overcome, so a correction is applied to avoid this
                                    ...
                                    if App.activate > 0: #evaluates if the app has some duty cycles to be considered
                                        if indexes_adj.size > 0:
                                            evaluate = np.round(np.mean(indexes_adj)) #calculates the mean time position of the current switch_on event, to later select the proper duty cycle
                                        else:
                                            evaluate = 0 
                                    ...
                                    if App.activate > 0:
                                        if indexes.size > 0:
                                            evaluate = np.round(np.mean(indexes))
                                        else:
                                            evaluate = 0
                                        if evaluate in range(App.cw11[0],App.cw11[1]) or evaluate in range(App.cw12[0],App.cw12[1]):
