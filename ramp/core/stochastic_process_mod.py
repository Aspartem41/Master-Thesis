...
import random 
import math
from ramp.core.initialise_mod import Initialise_model, Initialise_inputs

#%% Core model stochastic script

def Stochastic_Process(j,t,r, year, start_month, end_month, num_pro):
    Profile, num_profiles, Room_profile, Room_avg_list = Initialise_model(year, num_pro)
    peak_enlarg, mu_peak, s_peak, op_factor, Year_behaviour, Room_list = Initialise_inputs(j,t,r,year, start_month, end_month)
    '''
    ...
    Calculates first the overall Peak Window (taking into account all Room classes). 
    ...
    for Rm in Room_list:
        ...
        for App in Rm.App_list:
            ...
            single_wcurve = Rm.App_list[App_count].daily_use*np.mean(Rm.App_list[App_count].POWER)*Rm.App_list[App_count].number #this computes the curve for the specific App
            windows_curve = np.vstack([windows_curve, single_wcurve]) #this stacks the specific App curve in an overall curve comprising all the Apps within a Room class
            App_count += 1
        Rm.windows_curve = windows_curve #after having iterated for all the Apps within a Room class, saves the overall Room class theoretical maximum curve
        Rm.windows_curve = np.transpose(np.sum(Rm.windows_curve, axis = 0))*Rm.num_rooms
        Tot_curve = Tot_curve + Rm.windows_curve #adds the Room's theoretical max profile to the total theoretical max comprising all classes
    ...
    each Appliance instance within each Room instance is separately and stochastically generated
    '''

    for prof_i in range(num_profiles): #the whole code is repeated for each profile that needs to be generated
        Tot_Classes = np.zeros(1440)#initialise an empty daily profile that will be filled with the sum of the hourly profiles of each Room instance
        Profile_dict = {}
        Rooms_avg_dict = {}    ################################################################### NEW
        for Rm in Room_list: #iterates for each Room instance (i.e. for each Room class)
            # Tot_room_avg = np.zeros(1440)    ################################################################### NEW
            Indi_room_type_tot = np.zeros(1440)    ################################################################### NEW
            Rm.load = np.zeros(1440) #initialise empty load for Room instance
            Profile_dict[Rm.room_name] = []
            Rooms_avg_dict[Rm.room_name] = []     ################################################################### NEW
            for i in range(Rm.num_rooms):#iterates for every single room within a Room class. Each single room has its own separate randomisation
                daily_profile_tot = np.zeros(1440)
                if Rm.room_preference == 0:
                    rand_daily_pref = 0
                    pass
                else:
                    rand_daily_pref = random.randint(1,Rm.room_preference)
                for App in Rm.App_list: #iterates for all the App types in the given User class
                    ...
                    rand_window_4 = np.array([int(random.uniform((App.window_4[0]-App.random_var_4),(App.window_4[0]+App.random_var_4))),int(random.uniform((App.window_4[1]-App.random_var_4),(App.window_4[1]+App.random_var_4)))])
                    if rand_window_4[0] < 0:
                        rand_window_4[0] = 0
                    if rand_window_4[1] > 1440:
                        rand_window_4[1] = 1440
                    ...
                    if App.flat == 'yes': #if the app is "flat" the code stops right after filling the newly created windows without applying any further stochasticity
                        ...
                        App.daily_use[rand_window_4[0]:rand_window_4[1]] = np.full(np.diff(rand_window_4),App.POWER[prof_i]*App.number)
                        Rm.load = Rm.load + App.daily_use
                        continue
                        ...
                        App.daily_use[rand_window_4[0]:rand_window_4[1]] = np.full(np.diff(rand_window_4),0.001)
                    ...
                    if App.activate == 1:
                        ...
                        App.p_31 = App.P_31*(random.uniform((1-App.Thermal_P_var),(1+App.Thermal_P_var))) #randomly variates the power of thermal apps, otherwise variability is 0
                        App.p_32 = App.P_32*(random.uniform((1-App.Thermal_P_var),(1+App.Thermal_P_var))) #randomly variates the power of thermal apps, otherwise variability is 0
                        App.p_41 = App.P_41*(random.uniform((1-App.Thermal_P_var),(1+App.Thermal_P_var))) #randomly variates the power of thermal apps, otherwise variability is 0
                        App.p_42 = App.P_42*(random.uniform((1-App.Thermal_P_var),(1+App.Thermal_P_var))) #randomly variates the power of thermal apps, otherwise variability is 0
                        random_cycle1 = random.choice([np.concatenate(((np.ones(int(App.t_11*(random.uniform((1+App.r_c1),(1-App.r_c1)))))*App.p_11),(np.ones(int(App.t_12*(random.uniform((1+App.r_c1),(1-App.r_c1)))))*App.p_12))),np.concatenate(((np.ones(int(App.t_12*(random.uniform((1+App.r_c1),(1-App.r_c1)))))*App.p_12),(np.ones(int(App.t_11*(random.uniform((1+App.r_c1),(1-App.r_c1)))))*App.p_11)))]) #randomise also the fixed cycle
                        random_cycle2 = random.choice([np.concatenate(((np.ones(int(App.t_21*(random.uniform((1+App.r_c2),(1-App.r_c2)))))*App.p_21),(np.ones(int(App.t_22*(random.uniform((1+App.r_c2),(1-App.r_c2)))))*App.p_22))),np.concatenate(((np.ones(int(App.t_22*(random.uniform((1+App.r_c2),(1-App.r_c2)))))*App.p_22),(np.ones(int(App.t_21*(random.uniform((1+App.r_c2),(1-App.r_c2)))))*App.p_21)))])                    
                        random_cycle3 = random.choice([np.concatenate(((np.ones(int(App.t_31*(random.uniform((1+App.r_c3),(1-App.r_c3)))))*App.p_31),(np.ones(int(App.t_32*(random.uniform((1+App.r_c3),(1-App.r_c3)))))*App.p_32))),np.concatenate(((np.ones(int(App.t_32*(random.uniform((1+App.r_c3),(1-App.r_c3)))))*App.p_32),(np.ones(int(App.t_31*(random.uniform((1+App.r_c3),(1-App.r_c3)))))*App.p_31)))])#this is to avoid that all cycles are sincronous 
                        random_cycle4 = random.choice([np.concatenate(((np.ones(int(App.t_41*(random.uniform((1+App.r_c4),(1-App.r_c4)))))*App.p_41),(np.ones(int(App.t_42*(random.uniform((1+App.r_c4),(1-App.r_c4)))))*App.p_42))),np.concatenate(((np.ones(int(App.t_42*(random.uniform((1+App.r_c4),(1-App.r_c4)))))*App.p_42),(np.ones(int(App.t_41*(random.uniform((1+App.r_c4),(1-App.r_c4)))))*App.p_41)))])
                    else:
                        pass
                    ...
                    if rand_time > 0.99*(np.diff(rand_window_1)+np.diff(rand_window_2)+np.diff(rand_window_3)+np.diff(rand_window_4)):
                        rand_time = int(0.99*(np.diff(rand_window_1)+np.diff(rand_window_2)+np.diff(rand_window_3)+np.diff(rand_window_4)))
                    ...
                    while tot_time <= rand_time: #this is the key cycle, which runs for each App until the switch_ons and their duration equals the randomised total time of use of the App
                            ...
                            elif App.num_windows == 3:
                                switch_on = int(random.choice(np.concatenate((np.arange(rand_window_1[0],rand_window_1[1]),np.arange(rand_window_2[0],rand_window_2[1]),np.arange(rand_window_3[0],rand_window_3[1])),axis=0)))
                            else: 
                            ...
                            if App.daily_use[switch_on] == 0.001: #control to check if the app is not already on at the randomly selected switch-on time
                                ...
                                elif switch_on in range(rand_window_3[0],rand_window_3[1]): #if random switch_on happens in windows3, same code as above is repeated for windows3
                                ...
                                else: #if switch_on is not in window1, not in window2, not in window3 it shall be in window4. Same code is repreated
                                    if np.any(App.daily_use[switch_on:rand_window_4[1]]!=0.001):
                                        next_switch = [switch_on + k[0] for k in np.where(App.daily_use[switch_on:]!=0.001)]
                                        if (next_switch[0] - switch_on) >= App.func_cycle and max_free_spot >= App.func_cycle:
                                            upper_limit = min((next_switch[0]-switch_on),min(rand_time,rand_window_4[1]-switch_on))
                                        elif (next_switch[0] - switch_on) < App.func_cycle and max_free_spot >= App.func_cycle:
                                            continue
                                        else:
                                            upper_limit = next_switch[0]-switch_on
                                        ...
                                        upper_limit = min(rand_time,rand_window_4[1]-switch_on)
                                ...
                                if tot_time > rand_time: #control to check when the total functioning time is reached. It will be typically overcome, so a correction is applied to avoid this
                                    ...
                                    if App.activate > 0: #evaluates if the app has some duty cycles to be considered
                                        ...
                                        if evaluate in range(App.cw11[0],App.cw11[1]) or evaluate in range(App.cw12[0],App.cw12[1]):
                                        ...
                                        elif evaluate in range(App.cw31[0],App.cw31[1]) or evaluate in range(App.cw32[0],App.cw32[1]):
                                            ...
                                            np.put(App.daily_use,indexes_adj,(random_cycle4*coincidence))
                                            np.put(App.daily_use_masked,indexes_adj,(random_cycle4*coincidence),mode='clip')
                                    ...
                                    if App.activate > 0:
                                        ...
                                        if evaluate in range(App.cw11[0],App.cw11[1]) or evaluate in range(App.cw12[0],App.cw12[1]):
                                            ...
                                            np.put(App.daily_use,indexes,(random_cycle4*coincidence))
                                            np.put(App.daily_use_masked,indexes,(random_cycle4*coincidence),mode='clip')
                                    else:
                    ...
                    Rm.load = Rm.load + App.daily_use #adds the App profile to the Room load
                    daily_profile_tot = (daily_profile_tot + App.daily_use)#print('room completed')
                    Indi_room_type_tot = Indi_room_type_tot + daily_profile_tot
                Profile_dict[Rm.room_name].append(daily_profile_tot)
        ...
        Room_avg_list.append(Rooms_avg_dict)
        print('Profile',prof_i+1,'/',num_profiles,'completed') #screen update about progress of computation
    return(Profile, Room_profile, Room_list, room_load, Room_avg_list)