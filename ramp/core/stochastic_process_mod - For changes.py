...
def Stochastic_Process(j,t,r, year, start_month, end_month, num_pro):
    ...
    for prof_i in range(num_profiles): #the whole code is repeated for each profile that needs to be generated
        ...
        for Rm in Room_list: #iterates for each Room instance (i.e. for each Room class)
            ...
            for i in range(Rm.num_rooms):#iterates for every single room within a Room class. Each single room has its own separate randomisation
                ...
                Each_room_load = Profile_dict[Rm.room_name]
            Tot_room_avg = Rm.load/Rm.num_rooms
            Tot_Classes = (Tot_Classes + Tot_room_avg)
    ...
    return(Profile, Room_profile, Room_list, Each_room_load, Room_avg_list)