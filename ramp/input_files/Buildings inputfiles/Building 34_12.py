# -*- coding: utf-8 -*-
"""
Created on Tue Sep  5 09:57:47 2023
...
# LAB = Room("Laboratory",1,3)
# Room_list.append(LAB)

# OFF_1p = Room("Faculty Office - 1 person",1,3)
# Room_list.append(OFF_1p)

# OFF_2p = Room("Faculty Office - 2 person",1,3)
# Room_list.append(OFF_2p)

# OFF_5p = Room("Faculty Office - 5 person",1,3)
# Room_list.append(OFF_5p)

# STR = Room("Stairwells",1,3)
# Room_list.append(STR)

# HC_gf = Room("Hallways & Corridors - ground floor",1,3)
# Room_list.append(HC_gf)

# HC_ff = Room("Hallways & Corridors - first floor",1,3)
# Room_list.append(HC_ff)

# EL = Room("Elevator",1,3)
# Room_list.append(EL)
...
t2 = [1080,1440] # both t2 for hallways lights
t21 = [0,480]
...
# Variability factors
v1 = 0.1
v2 = 0.2   # For laptop time and For duty cycles
#%%

# Seminar Hall

SH = Room("Seminar Hall",10,3)
Room_list.append(SH)

# # Plug loads

SH_lights = SH.Appliance(SH,90,36,2,480,v1,90, fixed = 'yes', occasional_use = 1, wd_we_type = 0)
SH_lights.windows(t1,t2,v1)

SH_lights = SH.Appliance(SH,90,36,2,120,v1,90, fixed = 'yes', occasional_use = 1, wd_we_type = 1)
SH_lights.windows(t1,t2,v1)



# SH_projector = SH.Appliance(SH,1,30,1,60,0,60, wd_we_type = 0)
# SH_projector.windows(t1,t0,v1)

# SH_projector = SH.Appliance(SH,1,350,1,360,v1,90, wd_we_type = 0, occasional_use = 0.8)
# SH_projector.windows(t1,t0,v1)

# SH_projector = SH.Appliance(SH,1,350,1,120,v1,60, wd_we_type = 1, occasional_use = 0.2)
# SH_projector.windows(t1,t0,v1)

# SH_Lap = SH.Appliance(SH,1,300,1,120,v2,90, wd_we_type = 0, occasional_use = 0.8)
# SH_Lap.windows(t1,t0,v1)

# SH_Lap = SH.Appliance(SH,1,300,1,60,v2,60, wd_we_type = 1, occasional_use = 0.2)
# SH_Lap.windows(t1,t0,v1)
 
# SH_Lights= SH.Appliance(SH,18,36,1,150,v1,90, wd_we_type = 0, occasional_use = 0.7)
# SH_Lights.windows(t1,t0,v1)

# SH_Lights= SH.Appliance(SH,2,36,1,360,v1,90, wd_we_type = 0, occasional_use = 0.7)
# SH_Lights.windows(t1,t0,v1)
...
LH = Room("Lecture Hall",10,3)
Room_list.append(LH)

LH_lights = LH.Appliance(LH,90,36,2,480,v1,90, fixed = 'yes', occasional_use = 1, wd_we_type = 0)
LH_lights.windows(t1,t2,v1)

LH_lights = LH.Appliance(LH,4,36,2,480,v1,90, occasional_use = 1, wd_we_type = 1)
LH_lights.windows(t1,t2,v1)

# LH_projector = LH.Appliance(LH,1,30,1,60,0,60, wd_we_type = 0)
