...
df = pd.read_excel('input_files/Room inputfiles/Personal Room/Personal Room.xlsx', sheet_name = 'SS_Spring' , index_col=0)
...
# Light - Weekdays
PR_Light = PR.Appliance(PR,nl,l_ap,ntf2,l_ft_wd,v1,l_mt_wd, wd_we_type = 0, occasional_use = 0.65)
PR_Light.windows(t1,t3,v0)

# Light - Weekend
PR_Light = PR.Appliance(PR,nl,l_ap,ntf1,l_ft_we,v1,l_mt_we, wd_we_type = 4, occasional_use = 0.65)
...
fan_apt_wd = df.loc['Fan','APT-WD (mins)']
fan_apt_we = df.loc['Fan','APT-WE (mins)']
fan_spt_wd = df.loc['Fan','SPT-WD (mins)']
fan_spt_we = df.loc['Fan','SPT-WE (mins)']
fan_ft_wd = df.loc['Fan','FT-WD (mins)']
fan_ft_we = df.loc['Fan','FT-WE (mins)']
fan_mt_wd = df.loc['Fan','MT-WD (mins)']
fan_mt_we = df.loc['Fan','MT-WE (mins)']

# Fan - Weekdays
PR_fan = PR.Appliance(PR,nfan,fan_ap,ntf2,fan_ft_wd,v1,fan_mt_wd, wd_we_type = 0, occasional_use = 0.65)
PR_fan.windows(t1,t3,v0)

# Fan - Weekend
PR_fan = PR.Appliance(PR,nfan,fan_ap,ntf1,fan_ft_we,v1,fan_mt_we, wd_we_type = 4, occasional_use = 0.65)
