...
GPR = Room("Geo Preparation Room",N,RP)
Room_list.append(GPR)

df = pd.read_excel('input_files/Room inputfiles/Geo Preparation Room/Geo Preparation Room.xlsx', sheet_name = 'SS_Exam' , index_col=0)
...
# Lights - Weekdays
GPR_Lights = GPR.Appliance(GPR,nl,l_ap,ntf,l_ft_wd,v1,l_mt_wd, fixed = 'yes', wd_we_type = 0, occasional_use = 1)
GPR_Lights.windows(t1,t0,v0)

#LIghts - Weekend
GPR_Lights = GPR.Appliance(GPR,nl,l_ap,ntf,l_ft_we,v1,l_mt_we, fixed = 'yes', wd_we_type = 1, occasional_use = 0.2)
GPR_Lights.windows(t1,t0,v0)
